...
interface GISData {
  ...
  heatmap: {
    ...
    data: Array<{
      coordinates: { latitude: number; longitude: number };
      value: number; // defect density 0-100
      radius: number;
    }>;
  };
}
...
export class GISService {
   ...
   */
  static calculateZoneArea(coordinates: Array<{ latitude: number; longitude: number }>): number {
   ...
   */
  static analyzeByZone(
    zones: GISData['zones'],
    defects: GISData['defectLocations']
  ): GISData['zones'] {
 ...
 * GIS/Map Routes
 */
export const createGISRoutes = (db: any): Router => {
  const router = Router();
   ...
   */
  router.get('/:projectId/geojson', verifyToken, async (req, res) => {
    try {
      const { projectId } = req.params;
   ...
   * Get zone analysis
   */
  router.get('/:projectId/zones', verifyToken, async (req, res) => {
    try {
      const { projectId } = req.params;

      const zones: GISData['zones'] = [
        {
          id: 'zone-1',
          name: 'Main Facade',
          type: 'facade',
