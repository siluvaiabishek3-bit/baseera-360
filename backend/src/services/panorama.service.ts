...
interface PanoramaData {
  ...
  annotations: Array<{
    ...
    content: string;
    linkedTo?: string; // Link to annotation or other pano
  }>;
...
export class PanoramaService {
   ...
   */
  static async parsePanoramaMetadata(filePath: string): Promise<Partial<PanoramaData['image']>> {
  ...
  static createHotspot(
    position: { x: number; y: number; z: number },
    type: 'annotation' | 'defect' | 'measurement',
    content: string
  ): PanoramaData['annotations'][0] {
    return {
      id: `hotspot-${Date.now()}`,
      type: type as any,
      position,
  ...
  static calculateDistance(
    ...
    realWorldScale: number = 1 // meters per unit
  ): number {
 ...
 */
export const createPanoramaViewerRoutes = (db: any): Router => {
  const router = Router();
   ...
   * Get panorama data
   */
  router.get('/panorama/:panoId', verifyToken, async (req, res) => {
    try {
   ...
   */
  router.get('/panorama/:panoId/viewer', verifyToken, async (req, res) => {
    try {
      const { panoId } = req.params;
   ...
   */
  router.post('/panorama/:panoId/annotate', verifyToken, async (req, res) => {
    try {
      const { panoId } = req.params;
   ...
   * Create measurement in panorama
   */
  router.post('/panorama/:panoId/measure', verifyToken, async (req, res) => {
    try {
      const { panoId } = req.params;
