...
interface AnalyticsData {
  ...
  trends: {
    ...
    resolutionTrend: Array<{ date: string; percentage: number }>;
    severity: Array<{ date: string; critical: number; high: number; medium: number; low: number }>;
  };
  ...
  efficiency: {
    inspectionCoverage: number; // percentage
    dataQuality: number; // percentage
    defectDetectionRate: number; // defects per 1000m²
    averageTimePerZone: number; // hours
  };

  predictions: {
    estimatedDeteriorationRate: number; // % per year
  ...
  charts: {
    defectDistribution: Array<{ name: string; value: number; percentage: number }>;
    severityBreakdown: Array<{ severity: string; count: number; percentage: number }>;
    timelineData: Array<{ date: string; metrics: Record<string, number> }>;
  };
}
...
export class AnalyticsService {
  ...
  static generatePredictions(overview: AnalyticsData['overview']): AnalyticsData['predictions'] {
    ...
    return {
      ...
      recommendedInspectionFrequency:
        overview.defectsBySeverity.critical > 0 ? 'monthly' : 'quarterly',
  ...
  static generateComparison(overview: AnalyticsData['overview']): AnalyticsData['comparison'] {
    ...
    return {
      vs_industry_average: {
        ...
        repairCost:
          (this.calculateRepairCost(overview) / industryAvgCost) * 100,
      },
      vs_similar_buildings: {
        condition:
          overview.defectsBySeverity.critical > 3
            ? 'poor'
  ...
  static generateReport(defects: any[]): AnalyticsData {
    const overview: AnalyticsData['overview'] = {
      ...
      defectsByCategory: defects.reduce(
        (acc, d) => {
...
export const createAnalyticsRoutes = (db: any): Router => {
   ...
   * Get dashboard data
   */
  router.get('/:projectId/dashboard', verifyToken, async (req, res) => {
    try {
      const { projectId } = req.params;
   ...
   * Get AI predictions
   */
  router.get('/:projectId/predictions', verifyToken, async (req, res) => {
    try {
      const { projectId } = req.params;
